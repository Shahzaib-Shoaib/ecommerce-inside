...
export default function Input({ name, label, prefix, postfix,colorScheme, ...rest }: Props) {
  return (
    <Field<string | number> name={name} label={label} colorScheme={colorScheme}>
      {({ error, value = '', setValue }) => (
        <div className={`input  ${error && 'error'}`}>
          ...
          <input
            ...
            name={name}
            value={value}
            className="flex-1 px-3 py-1 border  rounded-sm  w-full focus:border-caribbeangreen hover:border-caribbeangreen outline-none"

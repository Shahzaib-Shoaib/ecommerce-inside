...
import { cookOptions } from '@/utils/helper';
    ...
    <Field name={name} label={label}>
      {({ value, error, setValue }) => (
        <$Select.Root {...rest} name={name} value={value} defaultValue={$default} onValueChange={setValue}>
          ...
          <$Select.Content className='bg-white  shadow-lg px-3 py-1 border  rounded-sm focus:outline-none'>
            <$Select.Viewport>
              {$options.map(({ label, value }) => (
                <$Select.Item
                  key={value}
                  value={value}
                  className="p-2  flex items-center hover:bg-slate-200 hover:text-caribbeangreen hover:outline-none  justify-between cursor-pointer duration-150 ring-2 ring-transparent ring-inset  radix-state-checked:font-medium radix-state-checked:text-brand-600"
                >
